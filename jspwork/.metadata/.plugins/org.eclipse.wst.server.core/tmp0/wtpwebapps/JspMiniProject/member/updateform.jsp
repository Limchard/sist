...
// 이메일 구분하기(@기준으로)
	// 방법 1
/* int idx=dto.getEmail().indexOf('@'); // 골뱅이 위치 구하기 	
String email1=dto.getEmail().substring(0, idx); // 0번부터 idx이전까지.. 즉 0~(idx-1)
String email2=dto.getEmail().substring(idx+1); // idx+1 번지부터 끝까지  */
	// 방법 2 (@을 구분자로 분리 )
StringTokenizer st=new StringTokenizer(dto.getEmail(),"@"); // @ 기준으로 구분하여 앞,뒤 값을 준다.
...
<body>
	<form action="member/updateproc.jsp" method="post" onsubmit="return check(this)"> <!-- onsubmit="return check(this) : submit 할때 check(this) 에 대해 한번 확인하고 submit 하자! 라는 뜻	 -->
	<input type="hidden" name="num" value="<%=num%>"> 
		<table class="table table-bordered" style="width: 500px;">
			...
			<tr>
				...
				<td>
					<input type="text" name="name" class="form-group" required="required" style="width: 120px;" placeholder="이름" value="<%=dto.getName()%>">
				</td>
			</tr>
			<tr>
				<th style="width: 100px; background-color: pink;">핸드폰</th>
				<td>
					<input type="text" name="hp" class="form-group" required="required" style="width: 200px;" placeholder="핸드폰번호" value="<%=dto.getHp()%>">
				</td>
			</tr>
			<tr>
				<th style="width: 100px; background-color: pink;">주소</th>
				<td>
					<input type="text" name="addr" class="form-group" required="required" style="width: 200px;" placeholder="주소" value="<%=dto.getAddr()%>">
				</td>
			</tr>
			<tr>
				<th style="width: 100px; background-color: pink;">이메일</th>
				<td>
					<input type="text" name="email1" id="email1" class="form-group" required="required" style="width: 120px;" placeholder="이메일" value="<%=email1%>">
					<b>@</b>
					<input type="text" name="email2" id="email2" class="form-group" required="required" style="width: 150px;" placeholder="이메일주소" value="<%=email2%>">
